...
class UtilityOrchestrationEngine:
        ...
        if any(word in request_lower for word in ['create database', 'make database', 'build database', 'new database']):
            workflow["steps"].append({
                ...
                "parameters": {
                    ...
                    "model": "auto_selected"  # Will be replaced with actual model during execution
                }
            })
    ...
    def create_database_step(self, step: dict, step_number: int, workflow_id: str):
        ...
        try:
            ...
            # Add default model selection for orchestration (since user can't select in workflow)
            # Use a reliable model for automated workflows
            parameters["model"] = self.get_default_model_for_workflow()
    ...
    def get_default_model_for_workflow(self):
        """Get a default model for automated workflow execution"""
        try:
            # Get available models from database service
            response = requests.get(
                f"{self.database_service_url}/api/models/status",
                timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                # Use qwen models for workflows (prefer qwen for SQL generation)
                preferred_models = [
                    'granite3.3:2b',      # Best for SQL generation - clean output
                    'qwen2.5:latest',     # Good qwen model for SQL
                    'phi3:latest',        # Reliable for SQL generation
                    'llama3.2:1b'         # Fast fallback
                ]
                
                for model in preferred_models:
                    if model in available_models:
                        logger.info(f"Using model '{model}' for workflow execution")
                        return model
                
                # If no preferred model found, use the first available
                if available_models:
                    logger.info(f"Using first available model '{available_models[0]}' for workflow execution")
                    return available_models[0]
            
            # Fallback to a common qwen model name
            logger.warning("No models available, using fallback qwen model name")
            return 'qwen2.5:latest'
            
        except Exception as e:
            logger.error(f"Error getting default model: {str(e)}")
            return 'qwen2.5:latest'  # Fallback to qwen model
